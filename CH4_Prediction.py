...
Original file is located at
    https://colab.research.google.com/drive/1n17gUB-O6fpiisVIxzy4o9M4pJr_3DW5
"""
...
data = pd.read_excel("Data-CH4.xlsx")
categorical_columns = ["AT", "ST", "CT"]

X = data.drop(columns=["CH4"])
y = data["CH4"]
...
def run_deep_learning_models(X, y):
    ...
    print("\n📁 Excel dosyası oluşturuldu: DNN_Results.xlsx")
...
def run_rnn_models(X, y):
    ...
    print("\n📁 Excel dosyası oluşturuldu: RNN_Results.xlsx")
...
def run_pure_cnn(X, y):
    ...
    # PRINT FORMAT
    # ============================
    print("\n===== PURE CNN MODEL – 10 Fold Results =====")
    print(f"R²: mean={r2_mean:.4f}, std={r2_std:.4f}, 95%CI=({round(r2_ci[0],1)}, {round(r2_ci[1],1)})")
    ...
    print("\n📁 Excel dosyası oluşturuldu: PURE_CNN_Results.xlsx")
...
def run_lstm_models(X, y):
    ...
    print("\n📁 Excel dosyası oluşturuldu: LSTM_Results.xlsx")
...
def build_cnn_lstm(input_dim):
    ...
    model.add(Conv1D(filters=128, kernel_size=3, activation='relu', padding='same'))  # Ek Conv1D katmanı
    model.add(LSTM(128, activation='tanh', return_sequences=True))  # Daha derin LSTM
    model.add(LSTM(64, activation='tanh', return_sequences=False))
    model.add(Dropout(0.2))  # Dropout eklenmesi
    model.add(Dense(32, activation='relu'))
    model.add(Dense(1, activation='linear'))
    model.compile(optimizer='adam', loss='mse')
    return model
...
def run_cnn_lstm_models(X, y):
    ...
    # PRINT FORMAT
    # ============================
    print("\n===== CNN-LSTM MODEL – 10 Fold Results =====")
    print(f"R²: mean={r2_mean:.4f}, std={r2_std:.4f}, 95%CI=({round(r2_ci[0],1)}, {round(r2_ci[1],1)})")
    print("RMSE: mean={:.4f}, std={:.4f}, 95%CI=({:.1f}, {:.1f})"
          .format(rmse_mean, rmse_std, rmse_ci[0], rmse_ci[1]))
    print("MAE : mean={:.4f}, std={:.4f}, 95%CI=({:.1f}, {:.1f})"
    ...
    print("\n📁 Excel dosyası oluşturuldu: CNN_LSTM_Results.xlsx")
...
plt.savefig("CH4_R2_RMSE.png", dpi=400, bbox_inches='tight')
...
plt.savefig("CH4_MAPE_MAE.png", dpi=400, bbox_inches='tight')
